<script lang="ts">
  ...
  import type { ToolType } from '$lib/state/tools.svelte';
  ...
  const severities: { value: 'critical' | 'minor' | 'suggestion'; label: string; color: string }[] = [
...
<div class="flex items-center gap-1 px-3 py-1.5 bg-[#111111] border-b border-white/[0.06] select-none">
  <!-- Tool buttons -->
  <div class="flex items-center gap-0.5 border-r border-white/[0.06] pr-2 mr-1">
    {#each tools as tool}
      ...
      >
        ...
        <!-- Shortcut hint -->
        <span class="absolute -bottom-0.5 -right-0.5 text-[8px] font-mono leading-none opacity-0 group-hover:opacity-60 transition-opacity">
          {tool.shortcut}
        </span>
      </button>
    {/each}
  ...
  <div class="flex items-center gap-0.5 border-r border-white/[0.06] pr-2 mr-1">
    {#each severities as sev}
      ...
      >
        <span class="w-1.5 h-1.5 rounded-full" style:background-color={sev.color}></span>
        {sev.label}
      </button>
    {/each}
  </div>
